...
def compute_matches(retrieved_all, ground_truth_info):
    ...
    for retr in retrieved_all:
        ...
        # if (retr in ground_truth_info[itr]):
            ...
            print('==> Query ' + str(itr) + ' retrieved the wrong reference ' + str(retr) +
                  ', the ground truth should be ' + str(ground_truth_info[itr][1]))
...
def compute_recall_rate_at_n_range(n, similarity_matrix, ground_truth_info):
    ...
    itr = 0
    for cnt in n_range:
        recall_rate_at_n_range[itr] = compute_recall_rate_at_n(cnt + 1, similarity_matrix, ground_truth_info)
        itr += 1
...
def get_query_img_name(dataset, cnt):
    if dataset == 'Gardens':
        ...
        return str(cnt) + '.png'
    elif dataset == 'ESSEX3IN1' or 'SPED' or 'CrossSeasons' or 'Pittsburgh' or 'Nordland':
...
def print_and_store_result(config, opt, total_time, refer_encoding_time, matching_time, similarity, method, parameters,
                           recall_rate_n_range):
    ...
    if config['print_precision'] is True:
        ...
        ground_truth_info = np.load(dataset_dir + 'ground_truth_new.npy', allow_pickle=True)
        matches, failed_cases = compute_matches(predictions, ground_truth_info)
        print('\n==> Precision @ 100% Recall: ' + str(np.sum(matches) / total_query_imgs) +
              '(' + str(np.sum(matches)) + '/' + str(total_query_imgs) + ')')
        recall_rate_at_n = compute_recall_rate_at_n_range(recall_rate_n_range, similarity, ground_truth_info)
    ...
    if config['store_result'] is True:
        ...
        if os.path.exists(result_root + 'result/' + dataset + '/' + method + '/' + time_str) is False:
            os.mkdir(result_root + 'result/' + dataset + '/' + method + '/' + time_str)

        with open(result_root + 'result/' + dataset + '/' + method + '/' + time_str + '/similarity', 'wb') as fp:
            pickle.dump(similarity, fp)
        fp.close()
        print("==> Similarity matrix saved.")

        with open(result_root + 'result/' + dataset + '/' + method + '/' + time_str + '/sorted_similarity', 'wb') as fp:
            pickle.dump(np.sort(similarity), fp)
        fp.close()
        print("==> Sorted similarity matrix saved.")

        with open(result_root + 'result/' + dataset + '/' + method + '/' + time_str + '/sorted_prediction', 'wb') as fp:
            pickle.dump(np.argsort(similarity), fp)
        fp.close()
        print("==> Sorted prediction saved.")

        with open(result_root + 'result/' + dataset + '/' + method + '/' + time_str + '/failed_cases', 'wb') as fp:
            pickle.dump(failed_cases, fp)
        fp.close()
        print('==> Failed cases saved.')

        with open(result_root + 'result/' + dataset + '/' + method + '/' + time_str + '/result.txt', 'w') as f:
            f.writelines('Dataset: ' + dataset + '\n')
            if dataset == 'Gardens':
                f.writelines('Sequence: ' + config['Dataset'][opt.dataset]['query_dir'].split('/')[2] + ' vs ' +
            ...
            f.writelines('Precision @ 100% Recall: ' + str(np.sum(matches) / total_query_imgs) +
                         '(' + str(np.sum(matches)) + '/' + str(total_query_imgs) + ')' + '\n')
            f.writelines("Recall Rate @ [1, 20]: " + str(recall_rate_at_n) + '\n')

            f.writelines('\n+++++++++++++++ Runtime Performance +++++++++++++++\n')
            f.writelines("Average time per query: " + str(total_time / total_query_imgs) + '\n')
            f.writelines("Average encoding time: " + str(refer_encoding_time / total_refer_imgs) + '\n')
            f.writelines("Average matching time per query: " + str(matching_time / total_query_imgs) + '\n')

            f.writelines("\n+++++++++++++++ Failed cases +++++++++++++++\n")
            for cnt in range(len(failed_cases)):
                f.writelines('Query ' + str(failed_cases[cnt][0]) +
                             ' retrieved the wrong reference ' + str(failed_cases[cnt][1]) +
                             ', the ground truth should be ' + str(failed_cases[cnt][2]) + '\n')
        f.close()
        print("==> Runtime performance saved.")

        print('==> Results are saved at ' + result_root + 'result/' + dataset + '/' + method + '/' + time_str)


def print_and_store_result_ii(config, total_query_imgs, predictions, dataset_dir, sp_root,
                              dataset_name, method, recall_rate_n_range):
    if config['print_precision'] is True:
        ground_truth_info = np.load(dataset_dir + 'ground_truth_new.npy', allow_pickle=True)
        matches, _ = compute_matches(predictions.T[0], ground_truth_info)
        print('Precision @ 100% Recall: ' + str(np.sum(matches) / total_query_imgs) +
              '(' + str(np.sum(matches)) + '/' + str(total_query_imgs) + ')')
        recall_rate_at_n = compute_recall_rate_at_n_range(recall_rate_n_range, predictions, ground_truth_info)
        print('Recall Rate @ [1, 20]: ' + str(recall_rate_at_n))
    ...
    if config['store_result'] is True:
        ...
        with open(sp_root + 'result/' + dataset_name + '/result_' + method + '.txt', 'w') as f:
            f.writelines('Precision @ 100% Recall: ' + str(np.sum(matches) / total_query_imgs) +
                         '(' + str(np.sum(matches)) + '/' + str(total_query_imgs) + ')' + '\n')
            f.writelines("Recall Rate @ [1, 20]: " + str(recall_rate_at_n) + '\n')
        f.close()
